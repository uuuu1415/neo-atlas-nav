...
export function WebsiteEditor({ website, categories, onClose, onSaved }: { website?: Website; categories: Category[]; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  async function save(event: React.FormEvent<HTMLFormElement>) {
    ...
    const data = { title: form.get("title"), url: form.get("url"), description: form.get("description"), categoryId: form.get("categoryId"), iconUrl: "", tags: String(form.get("tags")).split(/[,，]/).map((tag) => tag.trim()).filter(Boolean), pinned: website?.pinned ?? false };
  ...
  return <motion.div className="modal-backdrop" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}><motion.section className="modal glass" role="dialog" aria-modal="true" aria-labelledby="editor-title" initial={{ y: 20, scale: 0.97 }} animate={{ y: 0, scale: 1 }}><div className="modal-title"><div><small>CURATE YOUR ATLAS</small><h2 id="editor-title">{website ? "编辑网站" : "收藏一个好网站"}</h2></div><button type="button" aria-label="关闭" onClick={onClose}>×</button></div><form onSubmit={(event) => void save(event)}><label>网站链接<input autoFocus name="url" type="url" placeholder="https://example.com" defaultValue={website?.url} required /></label><label>网站名称<input name="title" placeholder="给它一个好记的名字" defaultValue={website?.title} required maxLength={160} /></label><label>描述<textarea name="description" placeholder="这个网站能帮你做什么？" defaultValue={website?.description} maxLength={1000} /></label><div className="form-row"><label>分类<select name="categoryId" defaultValue={website?.categoryId}>{categories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select></label><label>标签<input name="tags" placeholder="工具, 灵感" defaultValue={website?.tags.join(", ")} /></label></div>{error && <p role="alert" className="error">{error}</p>}<div className="modal-footer"><button type="button" className="secondary" onClick={onClose}>取消</button><button className="primary" disabled={busy}>{busy ? "正在保存…" : "保存网站 ↗"}</button></div></form></motion.section></motion.div>;
}
