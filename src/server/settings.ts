...
export class SettingsStore {
  ...
  private async readFile(file: string) { return settingsSchema.parse(JSON.parse(await readFile(file, "utf8"))); }
  ...
  private async load(): Promise<{ settings: Settings; warning?: string }> {
    try { return { settings: await this.readFile(this.file) }; }
    catch (error) {
      ...
      return { settings: backup, warning: "设置文件损坏，已恢复上一个有效版本；原文件已保留。" };
    }
  }
  ...
  save(input: unknown) {
    return this.serialize(async () => {
      ...
      if (current.revision !== proposed.revision) throw new SettingsConflictError("设置已被其他页面修改，请刷新后重试");
...
export const settingsStore = new SettingsStore(process.env.ATLAS_DATA_DIR ?? path.join(process.cwd(), "data"));
