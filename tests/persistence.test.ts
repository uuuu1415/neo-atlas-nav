...
async function directory() {
  const result = await mkdtemp(path.join(tmpdir(), "atlas-test-"));
...
describe("durable settings", () => {
  it("serializes conflicting writes and survives reopening", async () => {
    ...
    const results = await Promise.allSettled([
      store.save({ ...settings, name: "First" }),
      store.save({ ...settings, name: "Second" }),
    ]);
    ...
    await expect(store.save(settings)).rejects.toBeInstanceOf(
...
it("database schema migration is repeatable and persists data", async () => {
  ...
  first.db.insert(categories).values({ id: "work", name: "工作" }).run();
  first.sqlite.close();
  const second = openDatabase(dir);
  expect(second.db.select().from(categories).all()).toHaveLength(2);
  second.sqlite.close();
});
